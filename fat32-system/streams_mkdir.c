...
#include "streams_mkdir.h"

// Working until -mkdir now.
...
int writeDirtoPartition(FAT_SYSTEM* System, DENTRY* dirEntries, char* partitionName, TrackedBlocks* freebls) {
    unsigned char* buffer = malloc(sizeof(System->bootsector.blocksize));
    memset(buffer, 0, sizeof(System->bootsector.blocksize));
