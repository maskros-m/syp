...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<argc; i++) {
        ...
        if (strcmp(argv[i], "-add")==0) {
            ...
            // { strcpy(parentDir, "root"); printf("root\n"); }
        ...
        if (strcmp(argv[i], "-mkdir")==0) {
            ...
            char parentDir[32] = "root";
            //TODO: pathLevel & implement parentDir
            ...
            for (int i=0; i < MAX_DIR_ENTRIES; i++) {
                if (d->dirEntries[i].filename[0] == '\0') {
                    ...
                    if (blockCnt < freeBlocksLeft) {
                        ...
                        addNewDentry(userDir, dirSize, parentDir, dirEntry, i, 1);
                        TrackedBlocks freebls;
...
int checkValidPath(DENTRY* dirEntries, char** pathArray, int pathLevel) {
    // check if user file exists and any part of the path exists
