...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<argc; i++) {
        ...
        if (strcmp(argv[i], "-add")==0) {
            ...
            isFilenameProvided(filePath, "-add");

            int pathLevel = countFilenameInPath(filePath);
            char** pathArray = malloc(pathLevel*sizeof(char*));
            extractPath(filePath, pathArray);
            ...
            /* subdir code added Nov 14 --start*/
            walkDownDirPath(fileName, &System, dirEntry, pathArray, pathLevel, 0);
            // addDownDirPath ();
            free(pathArray);
            /* --end*/
            ...
            //             addNewDentry(userFile, _fileSize, dirEntry, i, 0);
        ...
        if (strcmp(argv[i], "-mkdir")==0) {
            ...
            for (int i=0; i < MAX_DIR_ENTRIES; i++) {
                if (d->dirEntries[i].filename[0] == '\0') {
                    ...
                    printf("size of folder: %ld\n", sizeof(System.rootDir.dirEntries));
                    int dirSize = sizeof(System.rootDir.dirEntries);
                    int blockCnt = getNumberOfBlocksTaken(dirSize, bootsector.blocksize);
                    int freeBlocksLeft = getNumberOfFreeBlocks(&System);

                    printf("Attempting to create folder [%s] to partition [%s], size=[%d]\n", userDir, fileName, dirSize);
                    ...
                    if (blockCnt < freeBlocksLeft) {
                        ...
                        addNewDentry(userDir, dirSize, dirEntry, i, 1);
                        TrackedBlocks freebls;
                        getArrayWithFreeBlocks(&System, dirSize, &freebls);
                        writeDirtoPartition(&System, newDirEntries, fileName, &freebls);
                        addUsedBlocksToFAT(&System, &freebls, i);
                        overwriteOldMetadata(fileName, &System);
...
void addNewDentry(char* userFile, int fileSize, DENTRY* dirEntries, int index, int isDir) {
...
int writeDirtoPartition(FAT_SYSTEM* System, DENTRY* dirEntries, char* partitionName, TrackedBlocks* freebls) {
    
    DENTRY dentryArr[MAX_DIR_ENTRIES] = {0};
    for (int i = 0; i<MAX_DIR_ENTRIES; i++) {
        strcpy(dentryArr[i].filename, dirEntries[i].filename);
        dentryArr[i].size = dirEntries[i].size;
        dentryArr[i].firstblock = dirEntries[i].firstblock;
        dentryArr[i].isDir = dirEntries[i].isDir;
    }
    printf("sizeof dentyArr = [%ld]\n", sizeof(dentryArr)); // 2816
    printf("sizeof buffer = [%d]\n", System->bootsector.blocksize);
    unsigned char* buffer = malloc(System->bootsector.blocksize*freebls->cnt);
    memset(buffer, 0, System->bootsector.blocksize);
    memcpy(buffer, dentryArr, sizeof(dentryArr));
    FILE* partitionPtr = fopen(partitionName, "rb+");
    int wCnt = 0;
    for (int i=0; i<freebls->cnt; i++) {
        // read a single block and copy its content into buffer array
...
void walkDownDirPath(char* partitionName, FAT_SYSTEM* System, DENTRY* dirEntriesPtr, char** pathArray, int pathLevel, int index) {
    if (index == pathLevel) {
        return;
    }
    for (int i=0; i<MAX_DIR_ENTRIES; i++) {
        if (strcmp(dirEntriesPtr[i].filename, pathArray[index])==0 && dirEntriesPtr[i].isDir) {
            /* added Nov 15 - implementing subdir */
            // Working bin file: test2.bin in backup_arr
            printf("directory [%s] found in this path at block [%d]\n", pathArray[index], dirEntriesPtr[i].firstblock);
            FILE* fp = fopen(partitionName, "rb");
            if (fp == NULL) { 
            ...
            DENTRY Dentries[MAX_DIR_ENTRIES] = {0};
            
            int offset = sizeof(FAT_SYSTEM) + System->bootsector.blocksize * dirEntriesPtr[i].firstblock;
            printf("offset: [%d], firstblock: [%d]\n", offset, dirEntriesPtr[i].firstblock);
            printf("size of fat system: [%ld], size of dirEntries: [%ld]\n", sizeof(FAT_SYSTEM), sizeof(Dentries));
            fseek(fp, offset, SEEK_SET);
            printf("checkpoint\n");
            int bytesCopied = fread(Dentries, 1, sizeof(Dentries), fp);
            printf("bytes successfully written = [%d]\n", bytesCopied);
            fclose(fp);
            dirEntriesPtr = Dentries;
            
            // for loop to test print from array
            printf("printing from [%s]\n", pathArray[index]);
            for (int i = 0; i<MAX_DIR_ENTRIES; i++) {
                if (Dentries[i].filename[0] != '\0') printf("[%d] => [%s]\n", i, Dentries[i].filename);
            }

            walkDownDirPath(partitionName, System, dirEntriesPtr, pathArray, pathLevel, index+1);
        }
        else if (strcmp(dirEntriesPtr[i].filename, pathArray[index])==0 && !dirEntriesPtr[i].isDir) {
            printf("file [%s] located in current dir [%s]\n", pathArray[index], dirEntriesPtr[i].filename);
        }
    }
}

void addDownDirPath(char* partitionName, FAT_SYSTEM* System, DENTRY* dirEntriesPtr, char** pathArray, int pathLevel, int index) {
    if (index == pathLevel) {
        return;
    }
    for (int i=0; i<MAX_DIR_ENTRIES; i++) {
